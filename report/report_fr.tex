...
\subsection{Récapitulatif des questions du sujet}

\begin{enumerate}
  \item \textbf{Quelles conjectures sont faciles à réfuter ?}\\
  Les plus faciles sont celles pour lesquelles une famille extrémale simple existe déjà dans la population initiale (petites cliques, chemins, araignées, doubles étoiles). Dans ces cas, la recherche locale trouve rapidement une violation positive.

  \item \textbf{Quels invariants semblent difficiles à manipuler ?}\\
  Les invariants les plus difficiles sont ceux liés aux distances, au spectral et à la domination (\texttt{largest\_distance\_eigenvalue}, \texttt{second\_smallest\_laplace\_eigenvalue}, \texttt{domination\_number}, \texttt{total\_domination\_number}, \texttt{independent\_domination\_number}), car ils sont sensibles à de petites modifications structurelles.

  \item \textbf{Quelles mutations sont efficaces ?}\\
  Les mutations locales sont efficaces pour explorer autour d'une bonne forme, mais elles sont surtout performantes quand elles partent de graines structurelles pertinentes. En pratique, leur rôle principal est l'optimisation fine autour de graphes déjà prometteurs.

  \item \textbf{L'architecture FunSearch améliore-t-elle réellement l'heuristique initiale ?}\\
  FunSearch améliore l'automatisation de la recherche de scores et produit des candidats valides atteignant 100/100. En revanche, dans nos expériences finales, elle n'améliore pas le temps total par rapport à la meilleure heuristique manuelle de la Partie 1.

  \item \textbf{L'IA a-t-elle produit des idées utiles ou seulement du code ?}\\
  L'IA a produit des idées utiles de scoring (guidage proche de la frontière, termes dérivés, ajustements structurels) et une boucle d'évolution exploitable. Cependant, les gains majeurs viennent surtout de la conception humaine des familles initiales et de la validation expérimentale.
\end{enumerate}
