def heuristic_score(G, invariants, conjecture):
    ...
    linear_like = set((
        ...
        "slater_number", "sub_total_domination_number"
    ))
    quadratic_like = set((
    ...
    dense = set(("clique_number", "triangle_number", "size", "density", "maximum_degree", "average_degree",
                 ...
                 "edge_connectivity", "vertex_connectivity", "energy", "laplacian_energy"))
    ...
    tree_like = set(("diameter", "radius", "domination_number", "total_domination_number", "independence_number",
                     "independent_domination_number", "wiener_index", "path_cover_number"))
    ...
    score += 2.48 * smooth_margin
    score += 0.66 * local_margin
    score += 0.08 * micro_margin
    score += 0.035 * size_margin
    score += 0.17 * near_positive

    score += 0.255 * high_norm - 0.108 * low_norm
    score += 0.125 * x_direction * deriv_mag * x_norm
    if sign == "<=":
        score += 0.042 * y_norm
    else:
        score -= 0.042 * y_norm
    ...
    if wanted_high in dense:
        score += 0.185 * density + 0.105 * triangles + 0.082 * max_deg_norm + 0.045 * avg_deg_norm + 0.022 * clique + 0.018 * chromatic
    if wanted_high in sparse:
        score += 0.148 * (1.0 - density) + 0.092 * leaves + 0.088 * max(diameter, radius) + 0.028 * isolated + 0.024 * independence + 0.018 * hub_hint
    if wanted_high in distance_like:
        score += 0.052 * (1.0 - density) + 0.034 * leaves + 0.025 * irregularity_hint + 0.018 * max(diameter, radius)
    if wanted_low in dense:
        score += 0.096 * (1.0 - density) + 0.046 * leaves + 0.031 * max(diameter, radius) + 0.018 * independence
    if wanted_low in sparse:
        score += 0.087 * density + 0.055 * max_deg_norm + 0.031 * triangles + 0.016 * matching + 0.012 * connectivity
    if wanted_low in distance_like:
        score += 0.043 * density + 0.026 * max_deg_norm + 0.012 * avg_deg_norm
    if wanted_high in tree_like and wanted_low in dense:
        score += 0.046 * (1.0 - density) + 0.036 * leaves + 0.012 * hub_hint
    if wanted_high in dense and wanted_low in tree_like:
        score += 0.046 * density + 0.036 * max_deg_norm + 0.012 * triangles

    subgroup = str(getattr(conjecture, "subgroup", "") or "").lower()
    if "tree" in subgroup:
        score += 0.132 * (1.0 - density) + 0.112 * leaves + 0.098 * diameter - 0.047 * triangles - 0.018 * isolated
    if "connected" in subgroup:
        score += 0.028 * min_deg_norm - 0.048 * isolated
    if "regular" in subgroup:
        score -= 0.067 * degree_var_hint + 0.026 * (1.0 - irregularity_hint)
    if "claw_free" in subgroup:
        score += 0.062 * density + 0.041 * triangles - 0.026 * leaves
    if "bipartite" in subgroup:
        score += 0.061 * (1.0 - min(1.0, 12.0 * triangles)) + 0.021 * (1.0 - density) - 0.012 * clique
