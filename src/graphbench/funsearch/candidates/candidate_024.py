...
def heuristic_score(G, invariants, conjecture):
    ...
    # Use a logistic-like weight that stays near 1 for small |violation|, decays smoothly
    ...
    if sign == '<=':
        ...
        deriv_bonus = -deriv / (abs(deriv) + 1.0)
    if violation < 0:
        # Continuous push that increases with distance from boundary but saturates
