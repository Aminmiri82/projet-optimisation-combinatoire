...
def heuristic_score(G, invariants, conjecture):
    ...
    # Logistic decay weight: stays near 1 for small violation, decays smoothly
    boundary_weight = 1.0 / (1.0 + 80.0 * v_sq)
    # Distance to boundary normalized to [0,1]
    diff_ratio = diff_abs / (diff_abs + n + 1.0)
    score += 6.0 * boundary_weight * diff_ratio

    # ---------- Gradient guidance when violation negative ----------
    # Direction: positive means moving toward satisfying inequality
    if sign == '<=':
        direction = deriv
    else:
        direction = -deriv
    ...
    if violation < 0:
        # Distance factor that saturates smoothly
        dist_factor = 1.0 - math.exp(-diff_abs / (n + 1.0))
        score += 1.2 * dir_norm * dist_factor
